...
<html lang="en">
	...
	<body class="tooltips">
		...
		?>
			...
			<h2 style="font-size:1.7em;color:green;text-transform:uppercase;text-align:center;">you are adding a subject</h2>
			<p>
            <center>
            <?php
			error_reporting(E_ERROR);
			...
			if($name && $category)
			{
				$check=mysqli_query($con,"select * from subject where name='$name'");
				$number=mysqli_num_rows($check);
				if($number > 0)
				{
					echo"subject already exists<br/><a href='exambasicsettings'>Go Back</a>";
				}
				else
				{
					$insert=mysqli_query($con,"insert into subject (name,category) values ('$name','$category')");
					if($insert)
					{
					...
					{
						echo"subject not added successfully<br/><a href='exambasicsettings'>Try Again</a>";
					}
				}
			}
			else
			{
                ...
                type subject name:<br/>
				<input style="width:70%;" type="text" name="name" placeholder="subject name goes here" /><br/><br/>
				select subject category:<br/>
				<select name="category" style="width:70%;">
				';
				$cate=mysqli_query($con,"select * from subjectcategory order by name");
