...
<html lang="en">
	...
	<body class="tooltips">
		...
		?>
			...
			if($studentid)
			{
			echo"<h1><u><b>SEARCH RESULT</b></u></h1><br/><br/>";
			    $check=mysqli_query($con,"select * from studentdetails where admissionnumber='$studentid'");
			...
			while($data=mysqli_fetch_array($check))
			{
				...
				echo"<form action='editstudentpage' method='post'>";
				echo"<input type='hidden' value='".$data['studentdetailsid']."' name='editstudent'/>";
				echo"<td><input style='background-color:#090;color:white;width:100%;' type='submit' value='edit student'/></td>";
				echo"</form>";
				$status=$data['status'];
					  if($status=="1")
					  {
					  ...
					  ';
					  $yellow=$data['boardingstatus'];
					  if($yellow=="oncampus")
					  {
						  $current="<font color='blue'>Boarding Student</font>";
						  $submut="<input type='submit' value='Make Offcampus' style='width:100%;background-color:purple;color:white;'/>";
					  }
					  elseif($yellow=="offcampus")
					  {
						  $current="<font color='purple'>Day Scholar</font>";
						  $submut="<input type='submit' value='Make Oncampus' style='width:100%;background-color:blue;color:white;'/>";
					  }
				      echo"<td>".$current."</td>";
					  echo'
			...
			{
				echo"There are no continuing students in the school database";
			}
			}
			else
			{
			    echo"important variable not passed on<br/><a href='viewstudent'>Try Again</a>";
			}
