		...
		?>
			...
			<br/><br/>
				<div class="container-fluid">
					...
					<h2 style="font-size:1.7em;color:green;text-transform:uppercase;text-align:center;">individual promotion of student progress</h2>
			<p>
			<center>
<?php
error_reporting(E_ERROR);
...
if($choice)
{
	...
	{
		echo"<fieldset>";
		echo"<legend>PROMOTION RESULTS BASED ON REQUEST</legend>";
		$username=$_POST['username']; 
		$nextclass=$_POST['nextclass'];
		if($username && $nextclass)
		{
		     $update=mysqli_query($con,"UPDATE studentdetails SET currentclass='$nextclass' WHERE  admissionnumber='$username'");
			 if($update)
			 {
			     echo"<font color='green'>Promotion of student ".$username." to class ".$nextclass." is successful</font>";
			 }
			 else
			 {
			     echo"promotion of student with id ".$username." to the selected class failed<br/><a href='exampromotestudent'>Try Again</a>";
			 }
		}
		else
		{
		     echo"necessary variable not passed in<br/><a href='exampromotestudent'>Try Again</a>";
		}
		echo"</fieldset>";
	}
	elseif($choice=="no")
	{
	    echo"<fieldset>";
		echo"<legend>CURRENTLY PROCESSING REQUEST</legend>";
		header("location:exampromotestudent");
		echo"</fieldset>";
	}
	else
	{
		echo"<br/><font color='grey'>choose an option above and submit choice</font>";
	}
